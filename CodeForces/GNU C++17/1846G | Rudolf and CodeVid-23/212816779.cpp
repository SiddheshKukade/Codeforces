...
void solve(int T)
{
    ...
    while(!pq.empty()){
        int distance, state;
        tie(distance, state) = pq.top();
 
        pq.pop();
 
        fr(i, 0, m){
            int newstate = state & edges[i].andmask;
            newstate = newstate | edges[i].ormask;
 
            if(d[newstate] > d[state] + edges[i].d){
                d[newstate] = d[state] + edges[i].d;
 
                pq.push({d[newstate], newstate});
            }
        }
    }
